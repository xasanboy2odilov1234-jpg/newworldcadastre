    ...
    >
      ...
      <LayoutGroup id="cadastre-brand">
        ...
        >
          <motion.div
            layout
            className={`flex items-center ${showBrandRow ? "flex-row gap-3" : "flex-col"}`}
            transition={isCompose ? composeTransition : wrapperTransition}
          >
            <motion.div
              layout
              className="relative flex shrink-0 items-center justify-center overflow-hidden rounded-full border-[3px] border-cadastre-blue bg-white"
              initial={
                reduced
                  ? { width: SMALL_CIRCLE, height: SMALL_CIRCLE, opacity: 1 }
            ...
            >
              <motion.div
                className="absolute inset-0 rounded-full bg-cadastre-blue"
                initial={{ scale: 0, opacity: 0.9 }}
                animate={{
                  scale: isReveal && !drawIcon ? [0, 1.08, 0] : 0,
                  opacity: isReveal && !drawIcon ? [0.9, 0.85, 0] : 0,
                }}
                transition={
                  reduced
                    ? { duration: 0.01 }
                    : { duration: 0.45, ease: EASE, times: [0, 0.65, 1] }
                }
              />

              <TachymeterDraw
                active={drawIcon || showBrandRow}
                reduced={reduced}
                className="relative z-10 size-14 text-cadastre-gold"
              />
            </motion.div>
            ...
            <motion.div
              ...
              initial={false}
              animate={{
                width: showBrandRow ? "auto" : 0,
                opacity: showBrandRow ? 1 : 0,
              }}
              transition={composeTransition}
            >
              <motion.div
                layout
                className="w-1 shrink-0 origin-top rounded-full bg-[#c9a227]"
                aria-hidden="true"
                initial={{ height: 0, opacity: 0 }}
                animate={{
                  height: showBrandRow ? 56 : 0,
                  opacity: showBrandRow ? 1 : 0,
                }}
                transition={
                  reduced
                    ? { duration: 0.01 }
                    : { duration: 0.55, ease: EASE, delay: 0.12 }
                }
              />
              ...
              <motion.div
                ...
                aria-hidden={!showBrandRow}
              >
                {BRAND_LINES.map((line, index) => (
                  <motion.span
                    key={line}
                    layout
                    className="text-sm font-bold leading-none tracking-[0.12em] text-cadastre-blue"
